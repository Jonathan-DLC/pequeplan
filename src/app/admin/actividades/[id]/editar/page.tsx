...
import { Actividad, Categoria, DiaSemana, Horario, Moneda, RangoEdad, Zona } from "@/lib/models";
...
export default function EditarActividad() {
  ...
  useEffect(() => {
    ...
    const act = new CatalogoAdminService().obtener(id);
    if (!act) { router.push("/admin"); return; }
  ...
  const guardar = () => {
    ...
    new CatalogoAdminService().editar(id, {
  ...
  return (
    <div>
      <h1 className="font-[family-name:var(--font-display)] text-2xl font-bold text-slate-800 mb-6">Editar Actividad</h1>
