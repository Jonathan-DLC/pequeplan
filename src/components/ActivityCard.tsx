...
export function ActivityCard({ actividad, categoria, zona }: Props) {
  ...
  return (
    <Link
      ...
      className="group block rounded-3xl bg-white p-5 shadow-md shadow-caribe-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-caribe-200/50"
    >
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
        {zona && (
          <span className="rounded-lg bg-arena-100 px-2 py-0.5">
            📍 {zona.nombre}
          </span>
        )}
        {horarioResumen && (
          <span className="rounded-lg bg-caribe-50 px-2 py-0.5">
            🕐 {horarioResumen}
          </span>
        )}
        <span className="ml-auto font-semibold text-sm text-coral-600">
          {actividad.esGratuita
            ? "Gratis"
            : `$${(actividad.precioDesde ?? 0).toLocaleString("es-CO")}`}
        </span>
      </div>
    </Link>
  );
